...
$userInfo = M('User')->where(array('id'=>$user_id))->field('pid,role,group_id')->find();
			...
			switch ($key) {
				...
				case 'payment':
					...
					foreach($payment as $key=>$vo){
						...
						$html .= "<label alizi-value='{$key}' alizi-target=':payment' alizi-fx='alizi.payment' alizi-fx-params='{$key}' class='ellipsis alizi-params alizi-payment-{$key} {$vo['classname']} ".($i==1?'active':'')."'><input type='radio' name='payment' value='{$key}' ".($i==1?'checked':'').">{$vo['name']}</label>";
						if($key=='creaditcard'){
