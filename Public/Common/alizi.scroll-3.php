<?php
$html = "<style>.alizi-scroll2{height:160px;overflow:hidden;line-height:30px;border-radius: 5px;margin:10px 5px;font-size:16px;}.alizi-scroll2 li span{margin-right:10px;}</style>";
...
if($aliziConfig['real_notice']==1){
	...
	for($i=0;$i<50;$i++){
		...
		$html .= "<li ".($i%2 == 0?"class='even'":'')."><span style='color:#0c3'>[最新订购]</span><span class='alizi-mobile'>{$nn}*({$mm})</span><span class='alizi-date'>{$time[$rand]}订购了</span><span class='alizi-item'>{$pro} <span style='color:#FF0000'>√</span></span></li>";
	}	
}
$html .= "</ul></div></div>";
