...
$params['status'] = array('name'=>'订单状态','request'=>true, 'checked'=>true);

$template['theme'] = $template['theme']?$template['theme']:'thin';
$html .= "<link href='".$aliziConfig['alizi_host'].C('ALIZI_ROOT')."Public/Alizi/theme/{$template['theme']}/alizi.css?v=".ALIZI_VERSION."' rel='stylesheet'>";
...
$html .= "<div class='alizi-order alizi-border alizi-lang-".C('DEFAULT_LANG')." clearfix' id='aliziOrder'><div class='alizi-main alizi-border {$show_notice}'><div class='alizi-title alizi-title-order alizi-border ellipsis'><i class='icon-cart'></i>{$info['name']}</div>";
		...
		$html .= "<div class='alizi-form-content alizi-border'><form action='".C('ALIZI_ROOT')."index.php?m=Order&a=adminBooking' method='post' id='aliziForm'><input type='hidden' name='lang' value='".C('DEFAULT_LANG')."'><input type='hidden' name='user_id' value='{$user_id}'><input type='hidden' name='user_pid' value='{$user_pid}'><input type='hidden' name='item_sn' value='{$info['sn']}'><input type='hidden' name='item_id' value='{$info['id']}'><input type='hidden' name='item_name' value='{$info['name']}'><input type='hidden' name='item_price' id='item_price' value='".($product?$product[0]['price']:$info['price'])."'><input type='hidden' name='url' value='{$url}'><input type='hidden' name='redirect' value='".($template['redirect_uri']?$template['redirect_uri']:$url)."'><input type='hidden' name='referer' value='{$referer}'><input type='hidden' name='order_page' value='{$request['page']}'><input type='hidden' name='channel_id' value='{$cookie['ac']}'><input type='hidden' name='qrcode_pay' value='{$info['qrcode_pay']}'><input type='hidden' name='math' value='{$paymentDefault['math']}'><input type='hidden' name='page' value='{$page}'><input type='hidden' name='alizi_token' value='{$token}'><input type='hidden' name='buy_num' value='{$info['buy_num']}'><input type='hidden' name='buy_num_decrease' value='{$info['buy_num_decrease']}'><input type='hidden' name='min_num' value='{$info['min_num']}'><input type='hidden' name='max_num' value='{$info['max_num']}'><input type='hidden' name='coupon_value' value='0'><input type='hidden' name='deposit' value='{$info['deposit']}'><input type='hidden' name='deposit_payment' value='{$info['deposit_payment']}'><input type='hidden' name='alizi' value='1'>";
		...
		foreach($params as $key=>$vo){
			...
			switch ($key) {
				...
				break;
				case 'price':
					//$html .= "<span class='alizi-shipping' ".($info['shipping_id']?'':"style='display:none;'")."><strong class='alizi-order-price'>0.00</strong>+<strong class='alizi-shipping-price'>0.00</strong>(".lang('shippingPrice').")=</span>".$symbol."<strong class='alizi-total-price'>".($product?$product[0]['price']:$info['price'])."</strong><strong class='alizi-coupon'></strong><span class='alizi-discount'></span>".$vo['info'];
					
					$html .= "<input type='number' id='alizi-price' name='total_price' placeholder='{$vo['info']}' autocomplete='off' alizi-request='{$vo['request']}' class='alizi-input-text' value='{$cookie[$key]}'>";
				...
				case 'payment':
					...
					foreach($payment as $key=>$vo){
						...
						$html .= "<label alizi-value='{$key}' alizi-target=':payment' alizi-fx-params='{$key}' class='ellipsis alizi-params alizi-payment-{$key} {$vo['classname']} ".($i==1?'active':'')."'><input type='radio' name='payment' value='{$key}' ".($i==1?'checked':'').">{$vo['name']}</label>";
						if($key=='creaditcard'){
				...
				case 'datetime':
					...
					$html .= "<input type='text' name='{$key}' placeholder='{$vo['info']}' autocomplete='off' class='alizi-input-text Wdate' alizi-request='{$vo['request']}' style='width:50%;' onfocus=\"WdatePicker({minDate:'$date'})\" value='{$cookie[$key]}'>
						<script type='text/javascript' src='{$aliziConfig['alizi_host']}__PUBLIC__/Assets/js/My97DatePicker/WdatePicker.js'></script>";
				break;
				case 'file':
				...
				break;
				case 'region':
					$regions = "<div class='region-select region-payOnDelivery'><select name='region[province]' id='province' class='alizi-region alizi-region-province' alizi-request='{$vo['request']}'></select><select name='region[city]' id='city' class='alizi-region alizi-region-city' alizi-request='{$vo['request']}'></select><select name='region[area]' id='area' class='alizi-region alizi-region-area' alizi-request='{$vo['request']}'></select></div>";
					...
					if(empty($aliziConfig['region'])){
						if(L('regions') && L('regions')!= 'REGIONS'){
							...
							$html .= "<select name='region[province]' id='province' class='alizi-region alizi-region-province' style='width:100% !important;'>{$region}</select>";
				...
				case 'status':
					...
					foreach($status as $k=>$v){
						$name = strip_tags($v);
						$status_option .= "<option value='{$k}'>{$name}</option>";
					}
					$html .= "<select name='status'>{$status_option}</select>";
				break;
				default:
